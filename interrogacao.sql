...
WHERE aluno.id_aluno = inscricao.id_aluno AND inscricao.data_ini <= datetime('now') 
	AND (datetime('now') <= inscricao.data_fim OR inscricao.data_fim IS NULL) AND inscricao.id_turma = turma.id_turma
	AND turma.id_turma = diretor.id_turma AND diretor.data_ini <= datetime('now') AND (datetime('now') <= diretor.data_fim 
...
WHERE area.id_area = turma.id_area AND turma.id_turma = inscricao.id_turma AND inscricao.id_aluno = aluno.id_aluno
	AND inscricao.data_ini <= datetime('now') AND (datetime('now') <= inscricao.data_fim OR inscricao.data_fim IS NULL)
...
.print ""
.print "8"
SELECT disciplina.id_disciplina, disciplina.codigo, disciplina.nome, periodo.numero AS periodo, ano_letivo.designacao AS ano_letivo,
	disciplina_aprovacoes.alunos_aprovados * 100 / disciplina_alunos.alunos_total AS pctg_aprovacoes
...
FROM pessoa AS p1, diretor, inscricao, alunos_negativa, responsavel, encarregado, pessoa AS p2
WHERE p1.id_pessoa = diretor.id_docente AND diretor.id_turma = inscricao.id_turma 
	AND inscricao.id_aluno = alunos_negativa.id_aluno AND alunos_negativa.id_aluno = responsavel.id_aluno
	AND responsavel.id_encarregado = encarregado.id_encarregado AND encarregado.id_encarregado = p2.id_pessoa;

.print ""
.print "10"
SELECT disciplina.id_disciplina, disciplina.nome, ano_letivo.designacao, pessoa.id_pessoa, pessoa.nome, temp.classificacao 
FROM disciplina, ano_letivo, frequencia, pessoa,
	(SELECT id_disciplina, frequencia.id_periodo, id_ano_letivo, max(classificacao) AS classificacao
...
.print ""
.print "11"
SELECT area.id_area, area.nome, pessoa.id_pessoa, pessoa.nome, pessoa.telefone, pessoa.telemovel
...
WHERE disciplina.id_disciplina = frequencia.id_disciplina AND frequencia.id_periodo = periodo.id_periodo 
...
SELECT docente.id_docente, ano_letivo.id_ano_letivo, count(*) AS alunos_aprovados
FROM docente, docencia, frequencia, periodo
WHERE docente.id_docente = docencia.id_docente AND docencia.id_disciplina = frequencia.id_disciplina
	AND frequencia.id_periodo = periodo.id_periodo AND frequencia.classificacao >= 10 AND periodo.numero = 3
GROUP BY disciplina.id_disciplina, frequencia.id_periodo;

DROP VIEW IF EXISTS docente_alunos;
CREATE VIEW docente_alunos AS
SELECT docente.id_docente, ano_letivo.id_ano_letivo, count(*) AS alunos_total
FROM docente, docencia, frequencia, periodo
WHERE docente.id_docente = docencia.id_docente AND docencia.id_disciplina = frequencia.id_disciplina
	AND frequencia.id_periodo = periodo.id_periodo AND periodo.numero = 3
GROUP BY disciplina.id_disciplina, frequencia.id_periodo;

.print ""
.print "14"
SELECT docente.id_docente, pessoa.nome, ano_letivo.designacao AS ano_letivo,
	docente_aprovacoes.alunos_aprovados * 100 / docente_alunos.alunos_total AS pctg_aprovacoes
